...
def demo_save_result(fname, x, y, z):
    """
...
def demo():
    ...
    # Load data from JSON file
    x_rec_face_np, y_rec_face_np, z_rec_face_np, w_rec_face_np, x_rec_skeleton_np, y_rec_skeleton_np, z_rec_skeleton_np, w_rec_skeleton_np, structure_skeleton = demo_load_data(fname_json)
    ...
    center, scale = compute_center_and_scale(
        [
            [[x_rec_face_np, w_rec_face_np], [x_rec_skeleton_np, w_rec_skeleton_np]],
            [[y_rec_face_np, w_rec_face_np], [y_rec_skeleton_np, w_rec_skeleton_np]],
        ]
    )
    # Normalize the x and y coordinates of both face and skeleton data
    x_rec_face_np = (x_rec_face_np - center[0]) / scale
    y_rec_face_np = (y_rec_face_np - center[1]) / scale
    x_rec_skeleton_np = (x_rec_skeleton_np - center[0]) / scale
    y_rec_skeleton_np = (y_rec_skeleton_np - center[1]) / scale    

    # Extract sizes. T_face should equal T_skeleton. The demo will fail otherwise.
    T_face = x_rec_face_np.shape[0]
    n_face = x_rec_face_np.shape[1]
    ...
    x_rec_face = torch.tensor(x_rec_face_np).to(device)
    y_rec_face = torch.tensor(y_rec_face_np).to(device)
    z_rec_face = torch.tensor(z_rec_face_np).to(device)
    w_rec_face = torch.tensor(w_rec_face_np).to(device)
    ...
    data_for_init_face = {
        ...
        "z_rec": z_rec_face,
        "w_rec": w_rec_face,
        "use_z_rec": True, # z-coordinates are fine for face
    }
    ...
    # loss functions
    loss_face = lambda x_cam, y_cam: wmse(x_cam, x_rec_face, w_rec_face) + wmse(y_cam, y_rec_face, w_rec_face)
    ...
    losses = [
        loss_face, # Loss function for the flexible face
        loss_face, # Loss function for the rigid face
        loss_skeleton,  # Loss function for the skeleton
    ]
    
    # Creating models
    model_object_face = ModelFlexibleObject(
        n=n_face,
        T=T_face,
        data_for_init=data_for_init_face,
        device=device,
    )
    model_object_skeleton = ModelSkeleton(
        T=T_skeleton,
    ...
    model_scene = ModelSceneMultipleObjects(
        models_object=[model_object_face, model_object_skeleton],
    ...
    # Result for flexible face 
    x_result_face = results_obj[0][0]
    y_result_face = results_obj[0][1]
    z_result_face = results_obj[0][2]
    
    # Result for skeleton 
    x_result_skeleton = results_obj[2][0]
    y_result_skeleton = results_obj[2][1]
    z_result_skeleton = results_obj[2][2]

    # denormalizing coordinates
    x_result_face = scale * x_result_face + center[0]  
    y_result_face = scale * y_result_face + center[1]  
    z_result_face = scale * z_result_face
    ...
    demo_save_result("result-face.txt", x_result_face, y_result_face, z_result_face)  
